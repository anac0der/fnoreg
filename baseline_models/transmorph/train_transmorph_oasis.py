...
import RaFD.TransMorph2D.losses as losses
from torch.utils.tensorboard import SummaryWriter
from RaFD.TransMorph2D.models.TransMorph import CONFIGS as CONFIGS_TM
import RaFD.TransMorph2D.models.TransMorph as TransMorph
...
config.img_size = (args.size, args.size + 32)
config.window_size = (config.img_size[0] // 32, config.img_size[1] // 32)
...
resize_transform = transforms.Resize(size=config.img_size)
resize_labels_transform = transforms.Resize(size=config.img_size, interpolation=transforms.InterpolationMode.NEAREST)
for epoch in range(init_epoch, train_config['epochs']):
    ...
    with torch.no_grad():
        ...
        for moving, fixed, moving_labels, fixed_labels in val_gen:
            ...
            x = torch.cat([moving, fixed], 1)
            X_Y, f_xy = model(x)
            warped_labels = reg_model([moving_labels.cuda().float(), f_xy.cuda()])   
