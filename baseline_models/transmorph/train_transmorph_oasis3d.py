...
config.img_size = (args.size, args.size + 32, args.size + 64)
config.window_size = (config.img_size[0] // 32, config.img_size[1] // 32, config.img_size[2] // 32)
...
for epoch in range(init_epoch, train_config['epochs']):
    ...
    for moving, fixed, moving_labels, fixed_labels in tqdm(train_gen, ncols=100):
        moving = torch.nn.functional.interpolate(moving, size=config.img_size, mode='trilinear')
        fixed = torch.nn.functional.interpolate(fixed, size=config.img_size, mode='trilinear')
        moving_labels = torch.nn.functional.interpolate(moving_labels, size=config.img_size, mode='nearest')
        fixed_labels = torch.nn.functional.interpolate(fixed_labels, size=config.img_size, mode='nearest')
        ...
        warp_labels = model.spatial_trans(moving_labels.float(), f_xy.float())
